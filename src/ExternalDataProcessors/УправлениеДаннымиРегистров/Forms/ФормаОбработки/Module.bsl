	...
	Для Каждого ТипРеквизита Из ТипыРеквизитов() Цикл
		...
		Для Каждого Реквизит Из ОбъектМетаданных[ТипРеквизита] Цикл
			...
			Если ЭтоРегистрБухгалтерии(ОписаниеРегистра) 
				И ЭтоСтандартныйРеквизит(ОписаниеРеквизита) 
			...
			Если ЭтоРегистрБухгалтерии(ОписаниеРегистра) Тогда				
				Если ЭтоИзмерение(ОписаниеРеквизита) Или ЭтоРесурс(ОписаниеРеквизита) Тогда
					ОписаниеРеквизита.УчетПоСчету = Не Реквизит.Балансовый;
				ИначеЕсли ЭтоСтандартныйРеквизит(ОписаниеРеквизита)
					И (ЭтоВидСубконтоПоИмени(Реквизит.Имя) Или ЭтоСубконтоПоИмени(Реквизит.Имя)) Тогда
	...
	Для Каждого КлючИЗапись Из ОписаниеРегистра.Реквизиты Цикл
		...
		Если ЭтоРегистрБухгалтерии(ОписаниеРегистра)
			...
			Для Каждого КорреспонденцияСчета Из КорреспонденцииСчетов Цикл
				...
				Если ЭтоСтандартныйРеквизит(ОписаниеРеквизита) 
				...
				ОписаниеПоля.ОписаниеРеквизита = ОписаниеРеквизита;
...
&НаКлиентеНаСервереБезКонтекста
Функция ЭтоСтандартныйРеквизит(ОписаниеРеквизита)
	Возврат ОписаниеРеквизита.Тип = "СтандартныеРеквизиты";
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ЭтоИзмерение(ОписаниеРеквизита)
	Возврат ОписаниеРеквизита.Тип = "Измерения";
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ЭтоРесурс(ОписаниеРеквизита)
	Возврат ОписаниеРеквизита.Тип = "Ресурсы";
КонецФункции
...
Функция НовоеОписаниеПоляЗаписи()
	...
	ОписаниеПоля.Вставить("ОписаниеРеквизита", НовоеОписаниеРеквизитаРегистра());
...
Функция ПолучитьОтобранныеЗаписи()
	...
	ЗаполнитьСтруктуруНастроекКомпоновкиДанныхДляВыборкиКлючевыхПолей(Настройки);
...
&НаСервере
Процедура ЗаполнитьСтруктуруНастроекКомпоновкиДанныхДляВыборкиКлючевыхПолей(Настройки)
	
	Настройки.Структура.Очистить();
		
	МассивИменПолей = Новый Массив;
	Если ОписаниеРегистра.ПодчинениеРегистратору Тогда
		МассивИменПолей.Добавить("Регистратор");
		МассивИменПолей.Добавить("НомерСтроки");
	КонецЕсли;
	
	Для Каждого Поле Из ОписаниеРегистра.ПоляЗаписи Цикл
		Если ЭтоИзмерение(Поле.ОписаниеРеквизита) Тогда
...
Процедура ОткрытьФормуВводаЗаписи(Действие, ДополнительныеПараметры = Неопределено)
	...
	Если Действие = "Добавить" Тогда
		ДанныеЗаписи = НоваяЗапись();
		Если ОписаниеРегистра.Реквизиты["Активность"] <> Неопределено 
			И ЭтоСтандартныйРеквизит(ОписаниеРегистра.Реквизиты["Активность"]) Тогда
			ДанныеЗаписи.Активность = Истина;
		КонецЕсли;
	...
	ИначеЕсли Действие = "УстановитьЗначенияВыделенные" 
		...
		ЗаголовокОкна = "Установка значений";
