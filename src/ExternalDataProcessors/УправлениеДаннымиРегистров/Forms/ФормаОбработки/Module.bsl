...
Функция ПолучитьПоля()
	...
	Для Каждого ПолеКомпоновки Из Список.КомпоновщикНастроек.Настройки.ДоступныеПоляВыбора.Элементы Цикл
		Если ПолеКомпоновки.Папка Тогда
			Продолжить;	
		КонецЕсли;
		
		Если ПолеКомпоновки.ТипЗначения.СодержитТип(Тип("ХранилищеЗначения")) Тогда
			Продолжить;
		КонецЕсли;
		
		ИмяПоля = Строка(ПолеКомпоновки.Поле);
		
		ОписаниеПоля = Новый Структура;
		ОписаниеПоля.Вставить("Имя", ИмяПоля);
		ОписаниеПоля.Вставить("Заголовок", ПолеКомпоновки.Заголовок);
		ОписаниеПоля.Вставить("ТипЗначения", ПолеКомпоновки.ТипЗначения);
...
Процедура ОткрытьФормуВводаЗаписи(Действие, ДополнительныеПараметры = Неопределено)
	...
	ПараметрыФормы.Вставить("ОписаниеПолей", ПолучитьПоля());
...
Процедура ДобавитьЗаписьНаСервере(ДанныеЗаписи) 
	...
	Если ОписаниеРегистра.ПодчинениеРегистратору Тогда
		...
		Замещать = ДанныеЗаписи.НомерСтроки > 0;
		...
		НаборЗаписей.ОбменДанными.Загрузка = ОбменДаннымиЗагрузка И Замещать;
