...
// Email: 	d.ivanov@1crg.ru
// Repo: 	https://bitbucket.org/st1vo/1c-epf-upravlenie-dannymi-registrov
// ********************** /
...
Процедура ИзменитьЗапись(Команда)
	...
	ОткрытьФормуВводаЗаписи("Изменить", Новый Структура("ДанныеЗаписи", ТекущиеДанные));	
	
КонецПроцедуры

&НаКлиенте
Процедура ЗаменитьЗначенияВыделенные(Команда)
	...
	ОткрытьФормуВводаЗаписи("ЗаменитьЗначенияВыделенные");	
	
КонецПроцедуры

&НаКлиенте
Процедура ЗаменитьЗначенияПоОтбору(Команда)
	
	ОткрытьФормуВводаЗаписи("ЗаменитьЗначенияПоОтбору");
	...
	Для Каждого КлючЗначение Из ТипыРегистров Цикл
		Для Каждого Регистр Из Метаданные[КлючЗначение.Ключ] Цикл
			...
			Если КлючЗначение.Значение = ТипыРегистров().РегистрыБухгалтерии Тогда
				...
				ОписаниеРеквизита.Вставить("ТипЗначения", Новый ОписаниеТипов("ПланСчетовСсылка." + Регистр.ПланСчетов.Имя));
				РеквизитыРегистра.Добавить(ОписаниеРеквизита);					
			КонецЕсли;
...
Процедура ОткрытьФормуВводаЗаписи(Действие, ДополнительныеПараметры = Неопределено)
	...
	ИначеЕсли Действие = "ЗаменитьЗначенияВыделенные" 
		Или Действие = "ЗаменитьЗначенияПоОтбору" Тогда
...
Процедура ПриЗакрытииФормыВводаЗаписиНаСервере(ДанныеЗаписи, ДополнительныеПараметры)
	...
	ИначеЕсли ДополнительныеПараметры.Действие = "ЗаменитьЗначенияВыделенные" Тогда
		
		ЗаменитьЗначенияНаСервере(ДополнительныеПараметры.ВыделенныеСтроки, ДанныеЗаписи);
					
	ИначеЕсли ДополнительныеПараметры.Действие = "ЗаменитьЗначенияПоОтбору" Тогда
		
		ЗаменитьЗначенияПоОтборуНаСервере(ДанныеЗаписи);
...
&НаСервере
Процедура ЗаменитьЗначенияПоОтборуНаСервере(НовыеЗначения)
	
	КоллекцияСтрок = ПолучитьОтобранныеЗаписи();
	ЗаменитьЗначенияНаСервере(КоллекцияСтрок, НовыеЗначения);
	
КонецПроцедуры

&НаСервере
Процедура ЗаменитьЗначенияНаСервере(КоллекцияСтрок, НовыеЗначения) 
	
	Если Не НовыеЗначения.Количество() Тогда
		Возврат;
	КонецЕсли;
	
	ДанныеЗаписи = ПолучитьПустуюСтруктуруПолей();
	
	Если ВыполнятьВТранзакции Тогда
		НачатьТранзакцию();
	КонецЕсли;
	
	Для Каждого ИсходнаяЗапись Из КоллекцияСтрок Цикл
		ЗаполнитьЗначенияСвойств(ДанныеЗаписи, ИсходнаяЗапись);
		ЗаполнитьЗначенияСвойств(ДанныеЗаписи, НовыеЗначения);
		
		ИзменитьЗаписьНаСервере(ДанныеЗаписи, ИсходнаяЗапись);
	КонецЦикла;
...
&НаКлиенте
Процедура ДекорацияРепозиторийНажатие(Элемент)
	ЗапуститьПриложение("https://bitbucket.org/1crg/1c-epf-upravlenie-dannymi-registrov");
КонецПроцедуры

&НаКлиенте
Процедура ДекорацияСайтНажатие(Элемент)
	ЗапуститьПриложение("http://1crg.ru");
КонецПроцедуры
